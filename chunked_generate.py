...
if __name__ == '__main__':
    ...
    # -------------------------------------------------------------------------
    # Define a function that saves the samples given to it as an HDF file
    # -------------------------------------------------------------------------

    def Save_HDF(results_list):
        ...
        samples['injection_samples'], injection_parameters['injection_samples'] = \
            zip(*results_list)

        # Sort all results by the event_time
        idx = np.argsort([_['event_time'] for _ in list(samples['injection_samples'])])
        samples['injection_samples'] = \
            list([samples['injection_samples'][i] for i in idx])
        injection_parameters['injection_samples'] = \
            list([injection_parameters['injection_samples'][i] for i in idx])
...
#START OF SAMPLE GENERATION BLOCK
    ...
    with tqdm(**tqdm_args) as progressbar:
        ...
        # Set a checkpoint, the samples will be saved every multiple of checkpoint (just in case)
        checkpoint = 50000
        increment = checkpoint
        # While we haven't produced as many results as desired, keep going
        while len(results_list) < n_samples:

            if(len(results_list) > checkpoint):
                print("\n",checkpoint,"samples have been generated, saving...")
                Save_HDF(results_list[:checkpoint])
...
#END OF SAMPLE GENERATION BLOCK

    print('Sample generation completed!\n')

    print("FINALLY TIME TO SAVE ALL SAMPLES")
    Save_HDF(results_list)
