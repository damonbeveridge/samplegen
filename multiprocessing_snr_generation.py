...
def snr_gen(queue,data,n_samples,i,det_string):
	...
	# Loop over all detectors
		...
		with tqdm(**tqdm_args) as progressbar:
			...
			for i in range(n_samples):
				...
				p = Process (target=snr_gen, args=(Q,df,n_samples,i,det_string))
				#p = Process(target=snr_gen,kwargs=dict(data=df,
				#	n_samples=n_samples,i=i,det_string=det_string,queue=Q))
				list_of_processes.append(p)

				p.start()

				snr_samples['injection_snr_samples'][det_string][i]=Q.get()
				snrs.append(snr_samples['injection_snr_samples'][det_string][i])
